...
const css = `
  *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
  ...
  .glow {
    ...
    background:radial-gradient(ellipse 60% 40% at 50% 0%,rgba(201,150,12,.06) 0%,transparent 60%);
  }
  ...
  .header {
    ...
    padding-bottom:28px; border-bottom:1px solid rgba(201,150,12,.15);
  ...
  .admin-badge {
    background:rgba(201,150,12,.12); border:1px solid rgba(201,150,12,.3);
    color:var(--gold); font-size:9px; font-weight:800; letter-spacing:3px;
    padding:5px 14px;
  }
  .welcome {
    font-family:'Great Vibes',cursive; font-size:36px; color:var(--gold);
    text-shadow:0 0 30px rgba(201,150,12,.3);
  }
  .logout {
    background:none; border:1px solid rgba(255,255,255,.1);
    color:var(--w35); font-family:'Montserrat',sans-serif;
  ...
  .stat-card {
    background:var(--w05); border:1px solid rgba(201,150,12,.12);
  ...
  /* Section title */
  .sec-title {
    font-family:'Bebas Neue',sans-serif; font-size:18px; color:var(--w50);
    letter-spacing:4px; margin-bottom:16px; margin-top:32px;
  }
  ...
  .day-pill:hover { border-color:var(--gold); background:rgba(201,150,12,.06); }
  .day-num { font-family:'Bebas Neue',sans-serif; font-size:20px; color:var(--gold); line-height:1; }
  .day-lbl { font-size:7.5px; color:var(--w35); letter-spacing:1px; margin-top:3px; }

  /* Footer */
  .admin-footer {
    border-top:1px solid rgba(201,150,12,.1); padding-top:24px;
    display:flex; align-items:center; justify-content:space-between;
...
const PAGES = [
  { icon:'🔐', name:'LOGIN PAGE',      desc:'User entry point',          href:'/' },
  { icon:'💳', name:'PRICING PAGE',    desc:'Plans & checkout',          href:'/pricing' },
  { icon:'👩', name:'ABOUT — MIA',     desc:'Welcome & intro',           href:'/about' },
  { icon:'⚖',  name:'DISCLAIMER',      desc:'Health & legal',            href:'/disclaimer' },
  { icon:'🔗', name:'START PAGE',      desc:'QR code hub',               href:'/start' },
  { icon:'📱', name:'APP PREVIEW',     desc:'Mobile app demo',           href:'/app-demo' },
]
...
export default function Admin() {
  ...
  const [session, setSession] = useState(true) // admin is logged in
    ...
    <>
      ...
      <div className="page">
        ...
        <div className="wrap">
          ...
          <div className="stats">
            {[
              { num:'28', lbl:'WORKOUT DAYS' },
              { num:'3',  lbl:'PRICING PLANS' },
              { num:'6',  lbl:'WEBSITE PAGES' },
              { num:'1',  lbl:'MEAL PLAN' },
            ].map(s => (
          ...
          <div className="pages-grid">
            {PAGES.map(p => (
              <div key={p.name} className="page-card" onClick={() => navigate(p.href)}>
          ...
          <p className="sec-title">28-DAY PROGRAM OVERVIEW</p>
          <div className="days-grid">
            {Array.from({length:28},(_,i) => (
              <div key={i} className="day-pill">
                <div className="day-num">{i+1}</div>
                <div className="day-lbl">{DAY_TYPES[i % 7]}</div>
              </div>
            ))}
          </div>
