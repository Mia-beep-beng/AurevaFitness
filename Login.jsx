...
const css = `
  ...
  /* ══ PHOTO PANEL ══════════════════════════════
     ...
     background:
       linear-gradient(to right,
         rgba(6,6,6,0) 0%, rgba(6,6,6,.65) 72%, #060606 100%),
       linear-gradient(to bottom,
         rgba(6,6,6,.45) 0%, transparent 20%,
         transparent 75%, rgba(6,6,6,.6) 100%),
       url('YOUR_PHOTO_URL') center / cover no-repeat;

     Recommended image prompt for ChatGPT (DALL-E):
     "Cinematic photo of a fit athletic woman
     viewed from behind, black strappy sports bra
     and leggings, toned back muscles, hair in a
     bun, walking toward a dramatic golden sunset,
     stone steps, dark moody foreground, black and
     gold palette, vertical portrait, photorealistic"
  ══════════════════════════════════════════════ */
  .photo-col {
    flex: 1.05; position: relative; overflow: hidden;
    background:
      linear-gradient(to right,
        rgba(6,6,6,0) 0%, rgba(6,6,6,.68) 74%, #060606 100%),
      radial-gradient(ellipse 60% 55% at 50% 45%,
        rgba(201,130,8,.22) 0%, rgba(160,90,4,.10) 40%, transparent 70%),
      radial-gradient(ellipse 80% 40% at 50% 90%,
        rgba(6,6,6,.9) 0%, transparent 70%),
      linear-gradient(160deg, #0d0a02 0%, #1a1100 45%, #060606 100%);
  }
  .photo-col::before {
    content:''; position:absolute; inset:0; pointer-events:none;
    background:
      radial-gradient(ellipse 50% 40% at 48% 42%,
        rgba(220,140,10,.18) 0%, transparent 60%),
      radial-gradient(ellipse 70% 25% at 50% 100%,
        rgba(6,6,6,.85) 0%, transparent 80%);
  }
  .photo-col::after {
    content:''; position:absolute; right:0; top:5%; height:90%; width:1px;
    background:linear-gradient(to bottom,
      transparent, rgba(201,150,12,.22) 30%,
      rgba(201,150,12,.22) 70%, transparent);
  }
  .photo-script {
  ...
  @keyframes pdrift {
    ...
    12% { opacity:1; }
    88% { opacity:.3; }
    100%{ transform:translate(14px,-85px); opacity:0; }
  }
  ...
  .l-brand {
    ...
    font-size:24px; color:var(--gold); letter-spacing:7px; line-height:1;
  }
  ...
  .eyebrow {
    ...
    opacity:0; animation:upfade .7s ease .5s forwards;
  }
  .welcome-title {
    font-family:'Cinzel',serif;
    font-size:clamp(24px,2.8vw,34px); font-weight:700; color:#fff;
    letter-spacing:3px; margin-bottom:10px;
    opacity:0; animation:upfade .7s ease .7s forwards;
  }
  .welcome-desc {
    font-size:13px; font-weight:300; color:var(--w50); line-height:1.7;
    opacity:0; animation:upfade .7s ease .85s forwards;
  }
  .form { display:flex; flex-direction:column; flex:1; justify-content:center; }
  .field { margin-bottom:20px; }
  .f-label {
    display:block; font-size:9.5px; font-weight:700;
    color:var(--w35); letter-spacing:2.5px; text-transform:uppercase; margin-bottom:8px;
  }
  ...
  .forgot-row { display:flex; justify-content:flex-end; margin:-6px 0 28px; }
  .forgot {
    background:none; border:none; color:rgba(201,150,12,.65);
    font-family:'Montserrat',sans-serif; font-size:10.5px; font-weight:500; padding:0;
  }
  .forgot:hover { color:var(--gold); }
  .cta {
    width:100%;
    background:linear-gradient(90deg, #A67408, #F0C832 48%, #A67408);
    background-size:220%; border:none; color:#040404;
  ...
  .cta:hover {
    animation:none; background-position:100%;
    transform:translateY(-2px); box-shadow:0 10px 36px rgba(201,150,12,.3);
  }
  ...
  .join {
    width:100%; background:transparent; border:1px solid rgba(201,150,12,.3);
    color:rgba(201,150,12,.8); font-family:'Montserrat',sans-serif;
    font-size:10px; font-weight:700; letter-spacing:3px;
    padding:14px; transition:all .3s; margin-bottom:20px;
  }
  ...
  .tagline {
    font-size:9.5px; font-weight:700; color:var(--w35); letter-spacing:5px;
    opacity:0; animation:upfade .7s ease 1.2s forwards;
  }
  @keyframes upfade {
  ...
  return (
    <>
      ...
      <div className="page">
        ...
        {/* Login column */}
        <div className={`login-col${ready ? ' in' : ''}`}>
          ...
          <div className="form">
            <div className="field">
              ...
              <input className="f-input" type="email" value={email}
                onChange={e => setEmail(e.target.value)} placeholder="you@example.com"/>
            </div>
            <div className="field">
              <label className="f-label">Password</label>
              <div className="f-wrap">
                <input className="f-input with-t"
            ...
            <button className="cta">
              <span>START TRAINING</span><span className="cta-arr">→</span>
            </button>
            <div className="or">
          ...
          <div style={{display:'flex',gap:'16px',justifyContent:'center',flexWrap:'wrap',marginTop:'16px'}}>
            {[['About','about'],['Disclaimer','disclaimer'],['@AurevaFitness','https://www.instagram.com/AurevaFitness']].map(([label,href])=>(
              <a key={label} href={href.startsWith('http')?href:`/${href}`}
                target={href.startsWith('http')?'_blank':undefined}
                rel={href.startsWith('http')?'noreferrer':undefined}
                style={{fontSize:'10px',color:'rgba(201,150,12,0.55)',textDecoration:'none',letterSpacing:'1px',fontFamily:'Montserrat,sans-serif'}}>
                {label}
              </a>
            ))}
          </div>
          <p style={{fontSize:'9px',color:'rgba(255,255,255,0.2)',textAlign:'center',letterSpacing:'1px',marginTop:'8px',fontFamily:'Montserrat,sans-serif'}}>
            © 2026 Aureva Fitness · All Rights Reserved
          </p>
        </div>
      </div>
    </>
